...
func (c *ChildController) Update(ctx *gin.Context) {
	id, _ := strconv.Atoi(ctx.Param("id"))
	var child models.Child
	if err := ctx.ShouldBindJSON(&child); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
	...
	child.ID = uint(id)
	if err := c.repo.Update(&child); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update child"})
		return
	}
	ctx.JSON(http.StatusOK, child)
}

func (c *ChildController) Delete(ctx *gin.Context) {
	id, _ := strconv.Atoi(ctx.Param("id"))
	if err := c.repo.Delete(uint(id)); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete child"})
		return
	}
